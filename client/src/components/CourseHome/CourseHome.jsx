...
export default function CourseHome() {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_APP_BASE_URL}/api/user/getCourseById`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleVideoComplete = () => {
    if (currentVideo < videoUrl.length - 1) {
      if (!check[currentVideo]) {
        fetch(
          `${import.meta.env.VITE_APP_BASE_URL}/api/user/handleCourseProgress`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleVideoCheck = (index) => {
    if (check[index]) {
      check[index] = false;
      fetch(`${import.meta.env.VITE_APP_BASE_URL}/api/user/handleCourseProgress`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
